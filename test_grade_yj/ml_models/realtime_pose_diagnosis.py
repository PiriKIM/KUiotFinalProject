...
def extract_features_from_landmarks(landmarks):
    # (train_grade_model2.py의 extract_features와 동일한 방식)
    # 필요한 랜드마크 인덱스
    try:
        # 좌표 추출
            ...
            if results.pose_landmarks:
                ...
                if features is not None:
                    try:
                        ...
                        cv2.putText(image, f"예측 오류: {e}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,255), 2)
            else:
                cv2.putText(image, "사람이 감지되지 않음", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
            
            cv2.imshow("실시간 자세 등급 인식", image)

...
class PoseGradeModelTrainer:
    ...
    def load_data(self):
        ...
        # auto_grade가 비어있는 경우 pose_grade를 사용하여 초기 라벨링
        # 실제 운영에서는 이 부분을 제거하고 auto_grade만 사용
        df['auto_grade'] = df['auto_grade'].fillna(df['pose_grade'])
        ...
        print(f"등급별 분포:")
        grade_counts = df['auto_grade'].value_counts()
        grade_names = {
            'a': 'A등급 (완벽)', 
            'b': 'B등급 (양호)', 
            'c': 'C등급 (보통)', 
    ...
    def extract_features(self, df):
        ...
        self.feature_names = [
            ...
            'neck_spine_interaction', 'total_asymmetry', 'overall_score',
            'neck_angle_squared', 'spine_angle_squared', 
            'shoulder_asymmetry_squared', 'pelvic_tilt_squared',
            'problem_count'
        ]
    ...
    def train_model(self, X, y):
        ...
        # 클래스별 정확도 확인
        print(f"\n클래스별 정확도:")
        for i, class_name in enumerate(['a', 'b', 'c', 'd', 'e']):
            if class_name in y_test:
    ...
    def plot_confusion_matrix(self, y_test, y_pred, save_path="confusion_matrix_grade.png"):
        ...
        plt.title('자세 등급 분류 혼동 행렬 (자동 등급)')
        plt.xlabel('예측 등급')
        plt.ylabel('실제 등급')
        plt.tight_layout()
    ...
    def predict_and_update_csv(self, csv_path=None):
        """CSV 파일의 데이터에 대해 예측하고 auto_grade 컬럼 업데이트 - 개선된 버전"""
        if self.model is None:
            print("모델이 훈련되지 않았습니다.")
            return
        ...
        for i, idx in enumerate(valid_indices):
            df.at[idx, 'auto_grade'] = predictions[i]
    ...
    def train_and_evaluate(self):
        ...
        # CSV 파일의 auto_grade 업데이트
        self.predict_and_update_csv()

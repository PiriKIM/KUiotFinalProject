...
# Tragus 위치 추정 (귀에서 좌측 + 하단 보정)
def estimate_tragus_xy_fixed(ear_landmark, image_w, image_h):
    x_offset = -0.02
    y_offset =  0.02
...
# ✅ C7 위치 최종 보정 함수 (더 왼쪽, 더 위쪽)
def estimate_c7_xy_final_adjusted(left_shoulder, right_shoulder, image_w, image_h):
    mid_x = (left_shoulder.x + right_shoulder.x) / 2
    mid_y = (left_shoulder.y + right_shoulder.y) / 2
    x_offset = -0.025  # 어깨 중간보다 더 안쪽
    y_offset = -0.07   # 어깨보다 더 위 (목 중앙 근접)
    x = mid_x + x_offset
    y = mid_y + y_offset
    return to_pixel(x, y, image_w, image_h)
...
while cap.isOpened():
    ...
    if result.pose_landmarks:
        ...
        # 보정된 좌표 계산
        tragus_xy = estimate_tragus_xy_fixed(tragus_landmark, w, h)
        c7_xy = estimate_c7_xy_final_adjusted(left_shoulder, right_shoulder, w, h)
    ...
    cv2.imshow('Posture Diagnosis (C7 adjusted)', frame)

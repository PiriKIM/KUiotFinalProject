...
while True:
    ...
    if result.pose_landmarks:
        ...
        if left_visible and not right_visible:
            ...
            # ⚠️ 귀가 둘 다 인식 안 됨 → 경고만 표시하고 계속 프레임 보여줌
            cv2.putText(frame, "🔍 귀가 인식되지 않음", (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (100, 100, 255), 2)
            cv2.imshow('Posture Diagnosis (v1.7.2a)', frame)
    ...
    cv2.imshow('Posture Diagnosis (v1.7.2 - 좌우 귀 감지 개선)', frame)
    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

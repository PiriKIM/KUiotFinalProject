# python3 merge_mp/ml_models/realtime_pose_classifier_grade.py
#    --model pose_classifier_4way_model.pkl
#    --right-csv merge_mp/data/right_side_angle_analysis.csv
#    --left-csv merge_mp/data/left_side_angle_analysis.csv

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
실시간 자세 등급 피드백 시스템 (ML 모델 기반 측면 판별)

python3 realtime_pose_classifier_grade.py --csv data/results/side_analysis_p1/side_angle_analysis.csv

웹캠으로 실시간 영상을 받아서 ML 모델로 측면을 판별하고 자세를 분석하여 등급(A/B/C)과 피드백을 화면에 표시합니다.
"""
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='실시간 자세 등급 피드백 시스템 (ML 모델 기반 측면 판별)')
    ...
    parser.add_argument('--model', '-m', default='pose_classifier_4way_model.pkl',
                       help='4way 자세 분류 모델 파일 경로')
    
    args = parser.parse_args()
    
    # ML 자세 분류기 초기화
    print("🤖 ML 자세 분류기를 초기화하고 있습니다...")
    pose_classifier = MLPoseClassifier(args.model)
    
    if not pose_classifier.is_trained:
        print("❌ ML 모델이 로드되지 않았습니다. 먼저 4way 모델을 학습시켜주세요.")
        return 1
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            if frame_count == 0:
                print(f"🎯 ML 모델이 감지한 측면: {detected_side}")
            
            # 측면에 따른 기준값 선택 (좌측면/우측면만)
            ...
            if detected_side in ['left', 'right']:
                ...
                # 정면이거나 측면을 감지할 수 없을 때
                stable_grade = 'N/A'
                cva_angle = 0.0
                message = "Front View Detected"
                color = (255, 255, 0)
            ...
            # 디버깅 정보 (처음 5프레임에서만)
            if frame_count <= 5:
                print(f"🔍 프레임 {frame_count}: ML측면={detected_side}, 등급={stable_grade}")
                if detected_side in ['left', 'right']:
                    print(f"   CVA 각도: {cva_angle:.1f}°")
                    print(f"   사용 기준값: {current_thresholds['min_abs']:.2f} ~ {current_thresholds['max_abs']:.2f}")
                if probability is not None:
                    confidence = max(probability)
                    print(f"   ML 신뢰도: {confidence:.1%}")
                print("-" * 40)
        
        # 화면 표시 (좌우반전)
        flipped_frame = cv2.flip(frame, 1)
        
        # 텍스트를 좌우반전된 화면에 다시 그리기 (좌우반전되지 않은 텍스트)
        flipped_frame = put_korean_text(flipped_frame, f"Camera: {args.camera} | ML-based Side Detection", 
                                       (10, 30), font_size=24, color=(255, 255, 255))
        
        if results.pose_landmarks:
            # ML 모델 감지 결과 표시
            if detected_side == 'front':
                side_color = (255, 255, 0)  # 노란색
                side_text = "ML: FRONT"
            elif detected_side == 'left':
                side_color = (255, 0, 0)    # 빨간색
                side_text = "ML: LEFT SIDE"
            elif detected_side == 'right':
                side_color = (0, 255, 0)    # 초록색
                side_text = "ML: RIGHT SIDE"
            else:
                side_color = (128, 128, 128)  # 회색
                side_text = "ML: UNKNOWN"
            
            flipped_frame = put_korean_text(flipped_frame, side_text, 
                                           (30, 160), font_size=24, color=side_color)
            
            # ML 모델 신뢰도 표시
            if probability is not None:
                confidence = max(probability)
                flipped_frame = put_korean_text(flipped_frame, f"ML Confidence: {confidence:.1%}", 
                                               (30, 190), font_size=20, color=(255, 255, 255))
            ...
            if detected_side in ['left', 'right']:
                # 등급 표시
                flipped_frame = put_korean_text(flipped_frame, f"Grade: {stable_grade}", 
                                               (30, 80), font_size=48, color=color)
                
                # CVA 각도 표시
                flipped_frame = put_korean_text(flipped_frame, f"CVA: {cva_angle:.1f}°", 
                                               (30, 130), font_size=32, color=(255, 255, 255))
                
                # 피드백 메시지 표시
                flipped_frame = put_korean_text(flipped_frame, message, 
                                               (30, 200), font_size=28, color=color)
            else:
                # 정면이거나 측면을 감지할 수 없을 때
                flipped_frame = put_korean_text(flipped_frame, "Front View Detected", 
                                               (30, 80), font_size=32, color=(255, 255, 0))
                flipped_frame = put_korean_text(flipped_frame, "Please turn to your side", 
                                               (30, 120), font_size=24, color=(255, 255, 255))
            
            # 프레임 카운트 표시
            flipped_frame = put_korean_text(flipped_frame, f"Frame: {frame_count}", 
                                           (30, 230), font_size=20, color=(255, 255, 255))
        else:
            # 랜드마크가 감지되지 않을 때
            flipped_frame = put_korean_text(flipped_frame, "No pose detected", 
                                           (30, 80), font_size=32, color=(0, 0, 255))
            flipped_frame = put_korean_text(flipped_frame, "Please stand in front of the camera", 
                                           (30, 120), font_size=24, color=(255, 255, 255))
        
        cv2.imshow('Real-time Posture Feedback (ML-based)', flipped_frame)

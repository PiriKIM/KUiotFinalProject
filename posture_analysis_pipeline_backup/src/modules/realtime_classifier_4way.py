        ...
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                if prediction is not None and probability is not None:
                    # 예측 결과 표시
                    pose_type = classifier.classes[prediction] if 0 <= prediction < 3 else "알 수 없음"
                    ...
                    detail_text1 = f"정면 확률: {front_prob:.1%}"
                    detail_text2 = f"좌측면 확률: {left_prob:.1%}"
                    detail_text3 = f"우측면 확률: {right_prob:.1%}"
